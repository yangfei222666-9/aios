#!/usr/bin/env python3
"""
集成系统测试
测试完整的语音唤醒 + 命令处理流程
"""

import os
import sys
import tempfile
import logging
from pathlib import Path

sys.path.insert(0, '.')

def setup_test_environment():
    """设置测试环境"""
    # 创建测试目录
    test_dir = tempfile.mkdtemp(prefix="voice_test_")
    print(f"测试目录: {test_dir}")
    
    # 创建必要的目录结构
    os.makedirs(os.path.join(test_dir, "notes"), exist_ok=True)
    os.makedirs(os.path.join(test_dir, "logs"), exist_ok=True)
    os.makedirs(os.path.join(test_dir, "tools"), exist_ok=True)
    
    # 创建模拟的 daily_check 脚本
    daily_check_script = os.path.join(test_dir, "tools", "daily_check_asr_tts.py")
    with open(daily_check_script, "w", encoding="utf-8", errors="replace") as f:
        f.write("""#!/usr/bin/env python3
# 模拟的 daily_check 脚本
import sys
print("✅ daily_check | ASR=OK TTS=OK NET=OK | search=test | top1=\"测试结果\"")
sys.exit(0)
""")
    
    return test_dir

def test_command_parsing():
    """测试命令解析"""
    print("\n命令解析测试")
    print("=" * 60)
    
    from tools.voice_command_handler import VoiceCommandHandler
    
    handler = VoiceCommandHandler()
    
    test_cases = [
        ("小九检查系统状态", "check_status", "检查系统状态"),
        ("添加笔记明天开会", "add_note", "添加笔记"),
        ("搜索人工智能", "search", "搜索信息"),
        ("今天天气怎么样", "weather", "查询天气"),
        ("现在几点", "time", "查询时间"),
        ("测试语音命令", "test", "测试命令"),
        ("有什么功能", "help", "显示帮助"),
    ]
    
    all_passed = True
    
    for cmd_text, expected_type, expected_desc in test_cases:
        cmd_type, cmd_info = handler.parse_command(cmd_text)
        
        if cmd_type == expected_type and cmd_info and cmd_info["description"] == expected_desc:
            print(f"[PASS] '{cmd_text}' -> {cmd_type} ({expected_desc})")
        else:
            print(f"[FAIL] '{cmd_text}'")
            print(f"  期望: {expected_type} ({expected_desc})")
            print(f"  实际: {cmd_type} ({cmd_info['description'] if cmd_info else 'None'})")
            all_passed = False
    
    return all_passed

def test_command_execution():
    """测试命令执行"""
    print("\n命令执行测试")
    print("=" * 60)
    
    test_dir = setup_test_environment()
    
    # 修改工作目录
    original_dir = os.getcwd()
    os.chdir(test_dir)
    
    try:
        from tools.voice_command_handler import VoiceCommandHandler
        
        handler = VoiceCommandHandler()
        
        # 测试添加笔记
        print("测试添加笔记命令...")
        success, message = handler.execute_command("添加笔记测试内容")
        
        if success:
            print(f"[PASS] 添加笔记成功: {message}")
            
            # 检查笔记文件
            note_file = os.path.join("notes", "inbox.md")
            if os.path.exists(note_file):
                with open(note_file, "r", encoding="utf-8") as f:
                    content = f.read()
                if "测试内容" in content:
                    print("[PASS] 笔记内容正确")
                else:
                    print("[FAIL] 笔记内容不正确")
            else:
                print("[FAIL] 笔记文件未创建")
        else:
            print(f"[FAIL] 添加笔记失败: {message}")
        
        # 测试状态检查
        print("\n测试状态检查命令...")
        success, message = handler.execute_command("检查系统状态")
        
        if success:
            print(f"[PASS] 状态检查成功: {message}")
        else:
            print(f"[FAIL] 状态检查失败: {message}")
        
        # 测试其他命令
        test_commands = [
            ("搜索测试", "search"),
            ("查询天气", "weather"),
            ("现在时间", "time"),
            ("测试命令", "test"),
            ("显示帮助", "help"),
        ]
        
        for cmd_text, expected_type in test_commands:
            success, message = handler.execute_command(cmd_text)
            status = "PASS" if success else "FAIL"
            print(f"[{status}] {cmd_text}: {message}")
        
        return True
        
    finally:
        # 恢复原始目录
        os.chdir(original_dir)
        
        # 清理测试目录
        import shutil
        try:
            shutil.rmtree(test_dir)
            print(f"\n已清理测试目录: {test_dir}")
        except:
            pass

def test_full_integration():
    """测试完整集成"""
    print("\n完整集成测试")
    print("=" * 60)
    
    # 模拟语音唤醒流程
    from tools.wake_listener import is_meaningful_command
    
    wake_phrases = ["小九", "你好小九", "小酒"]
    
    test_scenarios = [
        {
            "name": "完整有效命令",
            "audio": "小九检查系统状态",
            "should_process": True,
            "description": "唤醒词 + 有效命令"
        },
        {
            "name": "无意义命令",
            "audio": "你好小九",
            "should_process": False,
            "description": "寒暄 + 唤醒词"
        },
        {
            "name": "纯有效命令",
            "audio": "检查系统状态",
            "should_process": True,
            "description": "直接有效命令"
        },
        {
            "name": "添加笔记命令",
            "audio": "小九添加笔记测试内容",
            "should_process": True,
            "description": "唤醒词 + 笔记命令"
        },
        {
            "name": "搜索命令",
            "audio": "搜索人工智能",
            "should_process": True,
            "description": "搜索命令"
        },
    ]
    
    all_passed = True
    
    for scenario in test_scenarios:
        name = scenario["name"]
        audio = scenario["audio"]
        should_process = scenario["should_process"]
        description = scenario["description"]
        
        # 检查命令是否有意义
        is_meaningful = is_meaningful_command(audio, wake_phrases)
        
        # 判断结果
        passed = is_meaningful == should_process
        
        status = "[PASS]" if passed else "[FAIL]"
        print(f"{status} {name}")
        print(f"  音频: '{audio}'")
        print(f"  描述: {description}")
        print(f"  期望处理: {should_process}, 实际有意义: {is_meaningful}")
        
        if not passed:
            all_passed = False
        
        print()
    
    return all_passed

def main():
    """主测试函数"""
    print("语音唤醒系统完整集成测试")
    print("=" * 60)
    
    results = []
    
    results.append(("命令解析", test_command_parsing()))
    results.append(("命令执行", test_command_execution()))
    results.append(("完整集成", test_full_integration()))
    
    print("\n" + "=" * 60)
    print("测试结果汇总:")
    print("=" * 60)
    
    all_passed = True
    for test_name, result in results:
        status = "PASS" if result else "FAIL"
        print(f"{test_name}: [{status}]")
        if not result:
            all_passed = False
    
    print("\n" + "=" * 60)
    if all_passed:
        print("[SUCCESS] 所有集成测试通过！")
        print()
        print("系统功能总结:")
        print("1. [OK] 语音唤醒检测")
        print("2. [OK] 命令过滤器")
        print("3. [OK] 命令解析器")
        print("4. [OK] 命令执行器")
        print("5. [OK] 笔记管理")
        print("6. [OK] 状态检查")
        print("7. [OK] 完整集成")
        print()
        print("🎉 语音唤醒系统已完全集成，可以投入生产使用！")
        return 0
    else:
        print("[WARNING] 部分测试失败，需要进一步调整。")
        return 1

if __name__ == "__main__":
    sys.exit(main())