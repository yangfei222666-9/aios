...
### 口径不一致修复
- `engine.py`: emit 新增 `severity` 字段（CRIT/WARN/INFO），`_classify_severity()` 统一 fatal→CRIT
- `baseline.py`: snapshot 强制输出 `evolution_score` + `grade` + `severity_counts`，缺 key 标 `_schema_warning`
- `alerts.py`: 新增规则6（事件严重度/死循环检测）+ 规则7（执行器绕过检测）
- 修复前：CRIT=0 但有 fatal 事件；修复后：4 个 SEC 层 err 正确识别为 CRIT

### 通用执行器 executor.py
- `idempotency_guard(key, window=60)` — 同命令60秒去重
- `preflight_check(action, process_name)` — 进程存在检查
- `classify_error(err)` — RETRYABLE / NON_RETRYABLE 自动分类
- `execute(key, fn, ...)` — 一站式入口，串联去重→预检→执行→分类→终态回写
- 终态：SUCCESS / NOOP_ALREADY_RUNNING / NOOP_DEDUP / FAILED_RETRYABLE / FAILED_NON_RETRYABLE
- 执行日志 execution_log.jsonl + emit 同步到事件流
- 契约测试 17/17 PASS

### 别名归一化 app_alias.py
- `resolve(raw_text)` → canonical name + action + process_name + exe_path
- 繁简归一化（_T2S 映射表）
- ASR 错词别名：柯柯音乐/扣扣音乐/呵呵音乐/可可音乐 → QQ音乐
- `action_summary()` 生成用户可见摘要（"已关闭QQ音乐"）
- 测试 6/6 PASS

### 自我反省（12:14-12:27）
- 问题1：回复太啰嗦，把内部思考过程发给了用户
- 问题2：收到语音转写后没有自动执行，等用户提醒才做
- **同一个错误重复了5次**（12:11, 12:13, 12:16, 12:18, 12:24）
- 每次都是珊瑚海说"还是没打开/你没执行"后我才补执行
- 写了 AGENTS.md 规则、lessons.json、app_alias.py、executor.py，但行为没变
- 根因：这不是代码问题，是我的消息处理优先级问题——我总是先"思考和回复"再"执行"，应该反过来
- 教训：规则写了不等于执行了，行为改变比代码改变更难但更重要
