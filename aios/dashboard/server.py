...
class DashboardData:
    ...
    def get_system_health() -> Dict[str, Any]:
        ...
        recent = [e for e in events if datetime.fromisoformat(e.get("ts", "2000-01-01")) > last_hour]
        
        errors = [e for e in recent if e.get("level") == "ERROR"]
        warnings = [e for e in recent if e.get("level") == "WARN"]
        
        return {
            "status": "critical" if len(errors) > 10 else "warning" if len(errors) > 0 else "healthy",
...
async def acknowledge_alert(alert_id: str):
    ...
    try:
        ...
        if updated:
            ...
            return JSONResponse({"success": False, "message": "Alert not found or already acknowledged"}, status_code=404)
...
async def resolve_alert(alert_id: str):
    ...
    try:
        ...
        if updated:
            ...
            return JSONResponse({"success": False, "message": "Alert not found or already resolved"}, status_code=404)
    except Exception as e:
        return JSONResponse({"success": False, "message": str(e)}, status_code=500)
    ...
    try:
        result = subprocess.run(
            ["C:\\Program Files\\Python312\\python.exe",
             str(BASE_DIR / "agent_system" / "auto_dispatcher.py"), "heartbeat"],
            capture_output=True,
            text=True,
            timeout=30,
            encoding="utf-8",
