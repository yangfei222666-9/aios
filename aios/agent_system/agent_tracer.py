...
class AgentTracer:
    ...
    def start_task(self, task: str, context: Dict[str, Any] = None):
        """开始追踪一个任务"""
        # 自动推断 env（test Agent → test，其他 → prod）
        env = "test" if "test" in self.agent_id.lower() else "prod"
...
class TraceAnalyzer:
    ...
    def get_failure_patterns(self, min_occurrences: int = 3, env: str = "prod") -> List[Dict]:
        """识别重复失败模式（默认只看 prod）"""
        # 过滤环境
        filtered = [t for t in self.traces if t.get("env", "prod") == env] if env != "all" else self.traces
        failures = [t for t in filtered if not t.get("success")]
