...
from real_spawn import real_spawner
...
async def regenerate_low_success_agents():
    ...
    while True:
        ...
        if not failed_tasks:
            print("[OK] 没有失败任务，系统健康")
            # 模拟一个测试任务（演示用）
            if iteration <= 3:  # 前3次迭代执行测试任务
                print("[TEST] 执行测试任务（演示真实spawn）")
            ...
            print(f"[SCAN] 发现 {len(failed_tasks)} 个失败任务，开始重生...")
