...
class AutoDispatcher:
            ...
            if i < max_tasks:
                ...
                if result["status"] == "error":
                    ...
                    if retry_count < max_retries:
                        ...
                        self._log("warn", "Task retry scheduled", task_id=task.get("id"), retry=retry_count+1, max=max_retries)
                    else:
                        # 超过最大重试次数，记录失败
    ...
    def _dispatch_task(self, task: Dict) -> Dict:
        """分发单个任务到 Agent（通过 sessions_spawn）"""
        task_type = task.get("type", "monitor")
        message = task["message"]
        
        # 熔断器检查
        if self.circuit_breaker and not self.circuit_breaker.should_execute(task_type):
            retry_after = self.circuit_breaker.get_status().get(task_type, {}).get("retry_after", 300)
            self._log("warn", "Circuit breaker open", 
                     task_id=task.get("id"), 
                     task_type=task_type,
        ...
        try:
            ...
            self._log("info", "Spawn request created", 
                     task_id=task.get("id"), 
                     task_type=task_type,
                     model=template["model"],
                     label=template["label"])
            ...
            self._log("error", "Dispatch exception", 
                     task_id=task.get("id"), 
                     error=error_msg, 
                     task_type=task_type)
