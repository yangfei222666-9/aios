...
class AgentFallback:
    ...
    def get_fallback_strategy(self, error_type: str, retry_count: int) -> Optional[Dict]:
        """
        根据错误类型和重试次数返回降级策略
        """
    ...
    def _log_fallback(self, error_type: str, retry_count: int, strategy: Optional[Dict], status: str):
        """记录降级日志"""
        log_entry = {
