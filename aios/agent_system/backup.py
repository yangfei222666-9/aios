...
# 最小可恢复集合（MRS）定义
WORKSPACE_DIR = AGENT_SYSTEM_DIR.parent.parent  # C:\Users\A\.openclaw\workspace
...
def check_mrs_completeness() -> Tuple[bool, List[str], List[str]]:
    ...
    for category, config in MRS_DEFINITION.items():
        ...
        # 检查 workspace 级别的文件
        for file_path in config.get("workspace_files", []):
            full_path = WORKSPACE_DIR / file_path
            if full_path.exists():
                found_files.append(f"workspace:{file_path}")
            else:
                missing_files.append(f"workspace:{file_path}")
        
        # 检查 agent_system 内的模式匹配
        ...
        # 检查 workspace 级别的模式匹配
        for pattern in config.get("workspace_patterns", []):
            matched = list(WORKSPACE_DIR.glob(pattern))
            if matched:
                found_files.extend([f"workspace:{f.relative_to(WORKSPACE_DIR)}" for f in matched])
            else:
                missing_files.append(f"workspace_pattern:{pattern}")
...
def create_backup(check_only: bool = False) -> Dict:
    ...
    # 备份记忆与状态
    print(f"\n[3/4] 备份记忆与状态...")
    for file_path in MRS_DEFINITION["memory_and_state"].get("files", []):
        src = AGENT_SYSTEM_DIR / file_path
        if src.exists():
            dst = backup_dir / file_path
            dst.parent.mkdir(parents=True, exist_ok=True)
    ...
    # 备份 workspace 级别的文件（MEMORY.md 等）
    for file_path in MRS_DEFINITION["memory_and_state"].get("workspace_files", []):
        src = WORKSPACE_DIR / file_path
        if src.exists():
            dst = backup_dir / "workspace" / file_path
            dst.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(src, dst)
            backed_up_files.append(f"workspace/{file_path}")
            print(f"  ✓ workspace/{file_path}")
    
    # 备份 workspace 级别的模式匹配文件（daily logs 等）
    for pattern in MRS_DEFINITION["memory_and_state"].get("workspace_patterns", []):
        matched = sorted(WORKSPACE_DIR.glob(pattern), reverse=True)
        for memory_file in matched[:14]:  # 最近 14 天
            rel_path = memory_file.relative_to(WORKSPACE_DIR)
            dst = backup_dir / "workspace" / rel_path
            dst.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(memory_file, dst)
            backed_up_files.append(f"workspace/{rel_path}")
