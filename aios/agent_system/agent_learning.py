...
class AgentLearningLoop:
    ...
    def record_execution(self, task: str, result: Dict, success: bool, error: str = None):
        """
        记录 Agent 执行结果
        """
        execution_record = {
        ...
        with open(KNOWLEDGE_BASE, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # 匹配任务类型或通用教训
                    if lesson.get('task_type') == task_type or lesson.get('task_type') == 'general':
                        if lesson.get('confidence', 0) >= 0.5:  # 只返回置信度 >= 0.5 的教训
