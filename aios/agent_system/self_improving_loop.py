...
class SelfImprovingLoop:
    ...
    def __init__(self, data_dir: str = None):
        ...
        # 验证配置
        if hasattr(self.auto_rollback, 'VERIFICATION_WINDOW'):
            assert self.auto_rollback.VERIFICATION_WINDOW > 0, "VERIFICATION_WINDOW 必须大于 0"
    ...
    def _should_trigger_improvement(self, agent_id: str) -> bool:
        ...
        if last_improvement:
            try:
                ...
                self._log("error", f"无效的时间格式: {last_improvement}, 错误: {e}")
                # 清除无效的时间戳，允许继续
                self.state.get("last_improvement", {})[agent_id] = None
            else:
        ...
        for t in agent_traces:
            try:
                ...
                # 跳过时间格式错误或缺少 start_time 的 trace
                continue
    ...
    def _run_improvement_cycle(self, agent_id: str) -> int:
        ...
        if not agent:
            self._log("error", f"Agent {agent_id} 不存在，跳过改进")
            return 0
        ...
        for improvement in agent_improvements:
            # 验证必要字段
            required_fields = ["description", "risk"]
            if not all(field in improvement for field in required_fields):
                self._log("error", f"改进建议缺少必要字段: {improvement}")
                continue
