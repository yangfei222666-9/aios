...
class HealthMonitor:
    ...
    def _check_agents(self):
        ...
        for agent in agents:
            ...
            # disabled / deprecated / standby → 跳过，不参与健康分
            if status in ("disabled", "deprecated", "standby"):
                continue
            
            tasks_total = stats.get("tasks_total", 0)
            
            # tasks_total == 0 → never_run，不扣分，仅记录
            if tasks_total == 0:
        ...
        if never_run:
            warnings.append(f"{len(never_run)} 个 Agent 从未运行 (never_run): {', '.join(never_run[:5])}{'...' if len(never_run) > 5 else ''}")
