...
class SmartAgentPool(AgentPool):
    ...
    def spawn_spec(self, agent_id: str, template: str = "",
                   ...
                   max_tasks: int = 0, task_description: str = "") -> dict:
        """
        生成 spawn 规范，增加学习功能
        """
        # 1. 预检查：检查历史错误
        ...
        if lessons:
            ...
            enhanced_prompt = learning_loop.inject_lessons_to_prompt(base_prompt, task_description)
    ...
    def record_result(self, agent_id: str, task: str, result: dict, success: bool, error: str = None):
        """
        记录 Agent 执行结果，用于学习
        """
        learning_loop = AgentLearningLoop(agent_id)
    ...
    def handle_failure(self, agent_id: str, error: str, retry_count: int, current_config: dict) -> dict:
        """
        处理 Agent 失败，自动降级
