...
def ingest_manual(tool_stats: list):
    ...
    log_event("ingest", "gateway.collector",
...
if __name__ == "__main__":
    ...
    stats = [
        {"name": "exec", "ok": 45, "fail": 3, "avg_ms": 3000, "top_err": "command not found"},
        ...
        {"name": "web_search", "ok": 0, "fail": 2, "avg_ms": 0, "top_err": "ByteString encoding"},
        {"name": "image", "ok": 5, "fail": 0, "avg_ms": 200},
        {"name": "message", "ok": 3, "fail": 0, "avg_ms": 100},
        {"name": "cron", "ok": 2, "fail": 1, "avg_ms": 500, "top_err": "invalid params"},
    ]
    ingest_manual(stats)
