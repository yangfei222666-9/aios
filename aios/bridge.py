...
from core.engine import (
    emit, trace_span, log_tool_event,
    log_kernel, log_comms, log_mem, log_sec,
    LAYER_KERNEL, LAYER_COMMS, LAYER_TOOL, LAYER_MEM, LAYER_SEC,
)
...
def on_context_prune(tokens_before: int, tokens_after: int, strategy: str = "truncate"):
    """上下文裁剪事件"""
    return log_kernel("context_prune",
...
def on_agent_response(msg_len: int, channel: str = "telegram", latency_ms: int = 0):
    """Agent 回复发出"""
    return log_comms("agent_response", latency_ms=latency_ms,
...
def on_tool_not_found(tool_name: str, query: str = ""):
    """请求了不存在的工具（幻觉监测）"""
    return emit(LAYER_TOOL, "tool_not_found", "err", payload={
        "name": tool_name, "query": query[:200],
    })
...
def on_memory_recall(source: str, query: str, hit_count: int, latency_ms: int = 0):
    """成功检索到记忆/教训"""
    return log_mem("memory_recall", latency_ms=latency_ms,
                   source=source, query=query[:100], hit_count=hit_count)


def on_memory_store(source: str, topic: str, lesson_id: str = ""):
    """写入一条新记忆/教训"""
    return log_mem("memory_store", source=source, topic=topic[:100], lesson_id=lesson_id)
...
def on_runtime_error(source: str, error: str, traceback: str = ""):
    """Python 运行时错误"""
    return log_sec("runtime_error", status="err",
                   source=source, error=error[:300], traceback=traceback[:500])


def on_hallucination_detected(context: str, detail: str = ""):
    """检测到幻觉（后续步骤推翻前序结论）"""
    return log_sec("hallucination_detected", status="err",
                   context=context[:200], detail=detail[:300])


def on_ticket_created(ticket_id: str, title: str, level: str = "L2"):
    """自动生成工单"""
    return log_sec("ticket_created", ticket_id=ticket_id, title=title[:200], level=level)


def on_http_error(source: str, url: str, status_code: int, detail: str = ""):
    """HTTP 错误（502/401/404 等）"""
    return log_sec("http_error", status="err",
                   source=source, url=url[:200],
                   status_code=status_code, detail=detail[:200])
...
def on_match(query: str, matched_title: str, matched_id: str, score: float, match_type: str):
    """matcher 匹配后调用 → MEM 层"""
    return log_mem("match", source="aram.matcher",
                   query=query[:100], matched=matched_title,
                   matched_id=matched_id, score=score, match_type=match_type)


def on_correction(query: str, wrong_title: str, correct_title: str, correct_id: str):
    """用户纠正后调用 → MEM 层"""
    return log_mem("correction", status="err", source="aram.matcher",
                   query=query[:100], wrong=wrong_title,
                   correct=correct_title, correct_id=correct_id)


def on_confirm(query: str, matched_title: str, matched_id: str):
    """用户确认后调用 → MEM 层"""
    return log_mem("confirm", source="aram.matcher",
                   query=query[:100], matched=matched_title, matched_id=matched_id)
...
def on_retest(level: str, passed: int, failed: int):
    ...
    return emit(layer, "retest", "ok" if ok else "err", payload={
        "level": level, "passed": passed, "failed": failed,
    })


def on_lesson(title: str, category: str, status: str):
    """教训沉淀后调用 → MEM 层"""
    return log_mem("lesson_update", source="autolearn",
                   title=title[:100], category=category, lesson_status=status)


def on_health(source: str, ok: bool, checks: dict):
    """健康检查结果 → KERNEL 层"""
    return log_kernel("health_check", status="ok" if ok else "err",
