...
def get_dynamic_cooldown(playbook_id, base_cooldown_min):
    ...
    if rate < 0.5:
        # 失败率 > 50%，冷却翻倍
        adjusted = min(base_cooldown_min * DYNAMIC_COOLDOWN_MULTIPLIER, DYNAMIC_COOLDOWN_MAX)
        return int(adjusted)
    ...
    for pb in playbooks:
        ...
        if need_confirm and effective_mode != 'dry_run':
            entry = {
                ...
                "message": f"需要确认: {pb['name']}" + (" [全局熔断]" if fuse_tripped else ""),
...
def dashboard_metrics():
    ...
    if not REACTION_LOG.exists():
        return {"auto_exec_rate": 0, "verify_pass_rate": 0, "auto_close_rate": 0, "escalation_rate": 0, "total": 0}
            ...
            for line in f:
                ...
                try:
                    ...
                    if h.get('to') == 'RESOLVED':
                        total_resolved += 1
                        reason = h.get('reason', '')
                        if 'auto' in reason.lower() or 'reactor' in reason.lower() or 'converge' in reason.lower():
                            auto_closed += 1
                except:
                    continue
    ...
    return {
        ...
        "verify_pass_rate": f"{verify_passed/verify_total*100:.0f}%" if verify_total > 0 else "N/A",
        "auto_close_rate": f"{auto_closed/total_resolved*100:.0f}%" if total_resolved > 0 else "N/A",
...
def cli():
    if len(sys.argv) < 2:
        print("用法: python reactor.py [scan|dry_run|history|stats|metrics|fuse|playbook_stats]")
        return
    ...
    if cmd == 'scan':
        ...
        if not results:
            ...
            for r in results:
                icon = "✅" if r.get('status') == 'success' else "⚠️" if r.get('status') == 'pending_confirm' else "❌"
                print(f"{icon} [{r.get('playbook_id','?')}] {r.get('playbook_name','?')} → {r.get('status')}")
        ...
        for line in recent:
            ...
            print(f"{icon} {ts} [{r.get('playbook_id','?')}] → {r.get('status')} (decision:{did})")
        ...
        with open(REACTION_LOG, 'r', encoding='utf-8') as f:
            lines = f.readlines()
        total = len(lines)
        success = sum(1 for l in lines if '"success"' in l and '"partial_failure"' not in l)
        pending = sum(1 for l in lines if '"pending_confirm"' in l)
        failed = total - success - pending
        print(f"📊 响应统计: 总计={total} 成功={success} 待确认={pending} 失败={failed}")
