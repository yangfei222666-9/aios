...
def evaluate_click(target: ClickTarget) -> ClickDecision:
    ...
    # 综合判断
    all_passed = all(g["passed"] for g in [g1, g2, g3])
    failed_gates = [g["gate"] for g in [g1, g2, g3] if not g["passed"]]

    if all_passed:
        reason = "三道闸门全部通过，允许执行真点击"
    else:
        reasons = [g["reason"] for g in [g1, g2, g3] if not g["passed"]]
