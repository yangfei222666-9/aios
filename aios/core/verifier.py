...
def _verify_alert_gone(reaction, rule):
    ...
    active = [a for a in alerts.values()
...
def post_verify(verify_result, alert):
    ...
    if verify_result.get('passed'):
        ...
        try:
            ...
            reason = f"auto-resolved by reactor+verifier: {verify_result.get('detail','')}"
...
def cli():
    ...
    if cmd == 'history':
        ...
        for line in recent:
            ...
            ts = v.get('ts', '?')[:16]
            print(f"{icon} {ts} [{v.get('playbook_id')}] {v.get('verify_method')} → {v.get('detail','')[:60]}")
        ...
        print(f"📊 验证统计: 总计={total} 通过={passed} 失败={failed} 通过率={rate:.0f}%")
