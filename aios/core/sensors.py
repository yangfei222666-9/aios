...
def _is_cooled_down(state: dict, topic: str, key: str,
            ...
            try:
                ...
                if not reachable and prev_state:
                    ...
                    if _is_cooled_down(state, topic, target):
                        bus.emit(topic, {"target": target}, PRIORITY_HIGH, "network_probe")
                        _mark_fired(state, topic, target)
                elif reachable and not prev_state:
                    # 从不可达恢复
                    topic = "sensor.network.recovered"
                    bus.emit(topic, {"target": target}, PRIORITY_NORMAL, "network_probe")
...
def scan_all(watch_paths: list[str] = None,
    ...
    fw = FileWatcher(watch_paths or [
        ...
        str(Path(__file__).resolve().parent.parent.parent / "autolearn"),  # autolearn/
    ])
