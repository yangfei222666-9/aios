...
def cli():
    ...
    if cmd == 'analyze':
        ...
        for pid, p in patterns.items():
            ...
            print(f"  {icon} [{pid}]")
            print(f"      执行: {p['total']} (成功{p['success']} 失败{p['failed']} 待确认{p['pending']})")
            print(f"      成功率: {p['success_rate']:.0%} | 验证率: {p['verify_rate']:.0%}")
        ...
        for line in recent:
            ...
            ts = s.get('ts', '?')[:16]
            print(f"  {ts} [{s.get('playbook_id')}] {s.get('type')} — {s.get('reason','')[:60]}")
