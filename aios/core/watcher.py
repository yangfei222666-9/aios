...
class WatcherConfig:
    ...
    def __init__(self):
        ...
        # 检查间隔
        self.check_interval = int(cfg.get("watcher.check_interval_sec", str(DEFAULT_CHECK_INTERVAL)))
        
        # 阈值
        self.disk_pct_threshold = int(cfg.get("watcher.thresholds.disk_pct", str(DEFAULT_THRESHOLDS["disk_pct"])))
        self.memory_pct_threshold = int(cfg.get("watcher.thresholds.memory_pct", str(DEFAULT_THRESHOLDS["memory_pct"])))
        self.disk_free_gb_threshold = int(cfg.get("watcher.thresholds.disk_free_gb", str(DEFAULT_THRESHOLDS["disk_free_gb"])))
...
class SystemMonitor:
    ...
    def check(self):
        ...
        try:
            ...
            # 告警检查
            if disk_pct > self.config.disk_pct_threshold and self._should_alert("disk_pct"):
                alerts.append({
            ...
            if disk_free_gb < self.config.disk_free_gb_threshold and self._should_alert("disk_free"):
                alerts.append({
                    "type": "disk_space_low",
                    "severity": "CRIT",
                    "value": disk_free_gb,
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                if memory_pct > self.config.memory_pct_threshold and self._should_alert("memory_pct"):
                    alerts.append({
        ...
        # 发布告警事件
        for alert in alerts:
            priority = PRIORITY_CRITICAL if alert["severity"] == "CRIT" else PRIORITY_HIGH
            self.bus.emit(f"watcher.system.alert.{alert['type']}", alert, priority, "watcher")
        ...
        for target in self.config.network_targets:
            try:
                ...
                if not reachable and prev_state:
                    # 从可达变为不可达
                    self.bus.emit("watcher.network.unreachable", 
                                {"target": target}, PRIORITY_CRITICAL, "watcher")
                elif reachable and not prev_state:
                    # 从不可达恢复
                    self.bus.emit("watcher.network.recovered",
                                {"target": target}, PRIORITY_NORMAL, "watcher")
...
class ProcessMonitor:
    ...
    def check(self):
        ...
        for proc in appeared:
            self.bus.emit("watcher.process.started",
                        {"process": proc}, PRIORITY_NORMAL, "watcher")
        
        for proc in disappeared:
            self.bus.emit("watcher.process.stopped",
                        {"process": proc}, PRIORITY_HIGH, "watcher")
...
def main():
    ...
    parser.add_argument("--daemon", action="store_true", help="后台运行（Windows 服务模式）")
